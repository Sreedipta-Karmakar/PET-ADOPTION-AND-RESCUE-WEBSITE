...
function Adoption() {
  ...
  const filteredPets = pets.filter((pet: Pet) => {
    const locationMatch = pet.attributes.location
      .toLowerCase()
      .includes(searchQuery.toLowerCase());
    const speciesMatch =
      selectedSpecies === "" ||
      pet.attributes.species.data.attributes.name.toLowerCase() ===
        selectedSpecies.toLowerCase();
    const genderMatch =
      selectedGender === "" ||
      pet.attributes.gender.toLowerCase() === selectedGender.toLowerCase();
  ...
  return (
    <section className="page-wrapper" css={adoptionWrapper}>
      <h1 css={header}>Adoption</h1>
      <h2>
        Send mail here after adoption success- pawsandclawsmitra@gmail.com
      </h2>
