...
export default function Listing() {
  ...
  const [uploadImg] = useMutation<
    ...
    VariablesMutationUploadNewFile
  >(mutationUploadFile);
  const [createPet] = useMutation<unknown, VariablesMutationSetNewPet>(
    mutationSetNewPet
  );
      ...
      const mainFun = async () => {
        ...
        if (imgId) {
          ...
          const output = {
            inputs: inputs
              .filter((target) =>
                target?.type === "radio" ? target.checked : target.value
              )
              .map(({ value, id, name, type }) => ({
                [`${name || id}`]:
                  type === "number" ? Number(value.replace(/\,/gm, "")) : value
              }))
          };
          console.log({
          ...
          createPet({
            variables: {
              ...
              parentContact: Number(output.inputs[5].number),
              species:
                petEnum[output.inputs[0].species as keyof typeof petEnum],
              petAge: Number(output.inputs[1].age),
              email: String(output.inputs[6].email),
              location: String(output.inputs[7].location),
              breed: String(output.inputs[3].breed),
  ...
  return (
    <div className="page-wrapper" css={listingWrapper}>
      <span css={header}>List For Adoption</span>
